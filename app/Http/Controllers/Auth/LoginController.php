...
class LoginController extends Controller
{
    ...
    */
    ...
    protected function authenticated(Request $request, $user)
    {
        if ($user->status == 0) {
            $this->guard()->logout();
            back()->with('error', 'Your account has Disabled,Please contact Admin');
        } else {
            ...
            if ($role_id != 1) {
                $menus->whereHas('module_role', function ($query) use ($role_id) {
                    $query->where('role_id', $role_id);
                });

                $permissions->whereHas('permission_role', function ($query) use ($role_id) {
                    $query->where('role_id', $role_id);
                });
            }
